...
use ckb_testtool::{
    ckb_types::{core::TransactionView, packed, prelude::*},
    context::Context,
};
...
use spore_types::{
    ...
    spore::action::{
        Address, AddressUnion, BurnAgent, BurnProxy, BurnSpore, Byte32, Bytes, MintAgent,
        MintCluster, MintProxy, MintSpore, Script, SporeAction, SporeActionUnion, TransferAgent,
        TransferCluster, TransferProxy, TransferSpore,
    },
};

use super::spore_internal;
...
fn script_to_address(script: packed::Script) -> Address {
    ...
    let code_hash = h256_to_byte32(code_hash);
...
pub fn build_transfer_spore_action(context: &mut Context, nft_id: [u8; 32]) -> SporeActionUnion {
    let script = spore_internal::build_always_success_script(context, Default::default());
...
pub fn build_burn_spore_action(context: &mut Context, nft_id: [u8; 32]) -> SporeActionUnion {
    let from = spore_internal::build_always_success_script(context, Default::default());
